...
def warp_point(p):
    """Warp a point through ambient spacetime, exactly the grid's law
    (same radial factor per mouth, both superposed) but WITHOUT the mouth
    capture through render_vertex. A reference car that stays outside the
    throat spheres (r > b0) therefore bends around the wells and passes by
...
for frame_idx, progress in enumerate(progress_array):
    ...
    # Reference car: travels straight through flat space at y=4, bent only by
    # the ambient spacetime warp (same as the grid), so it passes the mouths.
    ref_triangles = []
    for i in range(len(orig_vectors)):
        tri = []
        for j in range(3):
            x_local, y_local, z_local = orig_vectors[i][j]
            flat = np.array([progress + x_local, y_local + 4.0, z_local])
            tri.append(warp_point(flat))
        ref_triangles.append(tri)

    all_triangles = processed_triangles + ref_triangles
...
print(f"Exported {num_frames} frames - wormhole car + reference car (y+4 offset)")
